    ...
    <main class="profile-page">
      ...
      <div v-else class="profile-content">
        ...
        <div class="cards-grid">
          ...
          <!-- Change Password -->
          <div v-if="editing || profile.is_temp_password" class="info-card full-span" ref="passwordCard" :class="{ 'card-urgent': profile.is_temp_password }">
...
export default {
  ...
  computed: {
    initials() {
      return (this.profile.fullName || 'A')
...
/* Temp banner */
.temp-banner  { display: flex; flex-direction: column; gap: 10px; background: #fef2f2; border-bottom: 1px solid #fecaca; padding: 12px 16px; flex-shrink: 0; }
...
/* Cards grid */
.cards-grid { display: grid; grid-template-columns: 1fr; gap: 14px; align-content: start; }

/* Info card */
.info-card { background: #fff; border-radius: 14px; border: 1px solid #e2e8f0; padding: 18px; position: relative; overflow: hidden; }
...
/* Lightbox */
.lightbox { position: fixed; inset: 0; z-index: 9999; background: rgba(0,0,0,0.88); display: flex; align-items: center; justify-content: center; backdrop-filter: blur(6px); }
.lb-img   { max-width: min(90vw, 480px); max-height: 85vh; border-radius: 14px; object-fit: contain; box-shadow: 0 32px 80px rgba(0,0,0,0.5); animation: lb-pop .22s cubic-bezier(.34,1.56,.64,1) both; }
@keyframes lb-pop { from { transform: scale(.88); opacity: 0; } to { transform: scale(1); opacity: 1; } }
.lb-close { position: absolute; top: 18px; right: 18px; width: 38px; height: 38px; border-radius: 50%; background: rgba(255,255,255,0.12); border: 1px solid rgba(255,255,255,0.2); color: #fff; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: background .15s; }
